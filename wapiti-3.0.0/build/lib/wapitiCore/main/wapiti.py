...
class Wapiti:
    ...
    def attack(self):
        ...
        for attack_module in self.attacks:
            ...
            # TODO: what do we mean by GET or POST: should I filter method of originals or generated requests ?
            generator = attack_module.attack(
                self.persister.get_links(attack_module=attack_module.name) if attack_module.do_get else [],
                self.persister.get_forms(attack_module=attack_module.name) if attack_module.do_post else []
            )
                    ...
                    if self._bug_report:
                        ...
                        try:
                            upload_request = Request(
                                "https://wapiti3.host-ed.me/upload.php",
...
def wapiti_main():
 ...
 ╚███╔███╔╝██║  ██║██║     ██║   ██║   ██║██████╔╝
  ╚══╝╚══╝ ╚═╝  ╚═╝╚═╝     ╚═╝   ╚═╝   ╚═╝╚═════╝  """
    ...
    print("Wapiti-3.0.0 (wapiti.sourceforge.net)")
    moon_phase = phase()
    if moon_phase == "full":
    ...
    parser = argparse.ArgumentParser(description="Wapiti-3.0.0: Web application vulnerability scanner")
